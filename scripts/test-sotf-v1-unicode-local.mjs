...
const connections=[];
let loader, initialized=false, store, ac, bc, rev=0, active='setup';
...
async function differential(label,candidate,expect='deny',host=ac,rpcClient=adb){
  ...
  const rpc=await rpcClient.rpc('sotf_v1_record_daily_brief_outcome',{outcome:candidate});
...
try{
  ...
  await check('I01 current workflow, exact version, bounded state, read-only business state',async()=>{
    ...
    const p=content(await call(ac,'sotf_get_daily_brief_state',stateInput));assert.equal(p.status,'ok');assert.equal(p.data.workspace_id,wa);assert.equal(p.data.state_revision,rev);assert.deepEqual(p.data.truncated_sections,[]);assert(Buffer.byteLength(JSON.stringify(p.data))<=65536);assert(!('events' in p.data));report.initialProjection=p.data;
  ...
  await check('I05 receipt read-back and no ordinary revision change',async()=>{const p=content(await call(ac,'sotf_get_daily_brief_state',stateInput));assert.equal(p.data.state_revision,rev);assert.equal(p.data.recent_outcomes[0].request_id,valid.request_id);});
  ...
  await check('I08 inverse ASCII truncation and safe replay after revision',async()=>{
    await append(step('a'.repeat(501)));const p=content(await call(ac,'sotf_get_daily_brief_state',stateInput));assert.deepEqual(p.data.truncated_sections,['commitments']);
  ...
  await check('N01 supplementary Unicode at truncation boundary',async()=>{
    ...
    report.reproducedBypass={utf16Units:text.length,unicodeScalars:Array.from(text).length,postgresCharacters:Number(sql(`select char_length(${quote(text)})`)),projection:p.data,dbDerived:derived,revision:rev};
  ...
  for(const row of corpus) await check(row.id+' '+row.description,async()=>{
    ...
    assert.deepEqual(projection.data.truncated_sections,sections);
    assert.deepEqual(semantics.truncated_sections,sections);
    if(text!=='')assert.equal(Buffer.byteLength(projection.data.commitments[0].definition_of_done,'utf16le')/2,row.prefixUnits);
    ...
    report.unicode.push({...row,measured,handlerTruncated:projection.data.truncated_sections,rpcTruncated:semantics.truncated_sections,expectedStateTruncated:row.truncated,canonical:'ACCEPT/ACCEPT',contradictory:'DENY/DENY',replay:'same receipt',emptySource:text===''?'empty eligible section; required source text cannot be empty':undefined});
  ...
  await check('U45 multiple projected sections use one measurement',async()=>{
    ...
    const p=content(await call(ac,'sotf_get_daily_brief_state',stateInput)).data;
