...
assert.equal(catalog.navigation.length,5);assert.equal(catalog.widgets.length,5);
...
const onlyCatalog=await api(only,"/api/bundles/layout");assert.equal(onlyCatalog.status,200);assert.equal(onlyCatalog.body.catalog.navigation.length,0);assert.equal(onlyCatalog.body.catalog.widgets.length,0);
assert.equal((await api(reader,"/api/bundles/layout")).status,403);
assert.equal((await reader.client.rpc("get_workspace_layout")).error?.code,"42501");pass("Experience-only empty state and unassigned API/RPC denial");
