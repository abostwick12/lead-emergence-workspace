...
async function request(path:string,subject:string|undefined,signal:AbortSignal,input?:object) {
 ...
 if(!response.ok)throw new ReadError(typeof result.message==="string"?result.message:"Investor is unavailable. Please retry.",response.status);
 return result;
}
export function useInvestorRead<T>(path:string,capability:string) {
 ...
 useEffect(()=>{
  ...
  void request(path,user?.id,controller.signal).then(data=>{if(live)setState({key,data,error:null});}).catch(error=>{
   if(!live)return;
   setState({key,data:null,error:controller.signal.aborted?"This request took too long. Try again; no saved work was changed.":error instanceof Error?error.message:"Investor is unavailable."});
...
export function useInvestorAction() {
 const {user,bundleExperience,refreshBundleExperience}=useWorkspace();
...
export function useUnsavedInvestor(dirty:boolean) {
 useEffect(()=>{
  if(!dirty)return;
  const before=(event:BeforeUnloadEvent)=>{event.preventDefault();};
  const click=(event:MouseEvent)=>{
   const link=(event.target as Element)?.closest?.("a[href]") as HTMLAnchorElement|null;
   if(link&&link.target!=="_blank"&&!link.hasAttribute("download")&&!event.ctrlKey&&!event.metaKey&&!event.shiftKey&&link.href!==window.location.href&&!window.confirm("Leave without saving your Investor changes?")){event.preventDefault();event.stopPropagation();}
  };
