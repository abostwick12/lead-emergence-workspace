...
export function ExecutiveDeliverySchedules({capability,availableKinds}:{capability:string;availableKinds:Kind[]}){
 ...
 const [editing,setEditing]=useState<Schedule|null>(null),[kind,setKind]=useState<Kind>(availableKinds[0]??"daily_brief"),
  [label,setLabel]=useState("Weekday daily brief"),[zone,setZone]=useState(browserZone),[localTime,setLocalTime]=useState("07:30"),
 ...
 const reset=(preferred?:Kind)=>{const next=preferred??creatable[0]??availableKinds[0]??"daily_brief";setEditing(null);setKind(next);setLabel(next==="daily_brief"?"Weekday daily brief":"Friday weekly review");setZone(browserZone());setLocalTime(next==="daily_brief"?"07:30":"16:00");setPolicy("when_attention_summary_changes");setWeekdays([5]);setConfirmed(false);setValidation(null);};
 ...
 const changeKind=(value:string)=>{const next=value as Kind;setKind(next);setLabel(next==="daily_brief"?"Weekday daily brief":"Friday weekly review");setLocalTime(next==="daily_brief"?"07:30":"16:00");setWeekdays([5]);setConfirmed(false);};
 useEffect(()=>{const next=creatableKey.split(",")[0] as Kind|undefined;if(!editing&&next&&next!==kind)changeKind(next);},[creatableKey,editing,kind]);
 const submit=async(event:React.FormEvent)=>{event.preventDefault();const definition={schemaVersion:"1.0",deliveryKind:kind,label,timeZone:zone,cadence:kind==="daily_brief"?{kind:"daily",localTime}:{kind:"weekly",localTime,weekdays},changePolicy:policy,deliveryTarget:"native_executive_inbox"};
 ...
 return <section className={styles.section} aria-labelledby="executive-delivery-heading"><p className={styles.eyebrow}>Native review rhythm</p><h2 id="executive-delivery-heading">Bring the right review back at the right time</h2>
  ...
  {read.loading||read.error?<ReadState loading={read.loading} error={read.error} retry={read.retry}/>:read.data&&<>
   ...
   {(editing||creatable.length>0)&&<form onSubmit={submit} className={styles.scheduleEditor}><h3>{editing?"Edit "+kindLabels[kind].toLowerCase():"Add a native review schedule"}</h3>
    {!editing&&creatable.length>1&&<Choice label="Review" value={kind} values={creatable} onChange={changeKind} labels={kindLabels}/>}<div className={styles.grid}><Field label="Schedule name" value={label} onChange={setLabel} max={120} required/><Field label="Named time zone" value={zone} onChange={setZone} max={100} hint="Use a named zone such as America/Chicago so daylight-saving changes are handled." required/><Field label="Local time" type="time" value={localTime} onChange={setLocalTime} required/><Choice label="When to create a cue" value={policy} values={["when_attention_summary_changes","always"]} onChange={setPolicy} labels={{when_attention_summary_changes:"Only when attention changes",always:"At every occurrence"}}/></div>
    {kind==="weekly_review"&&<fieldset><legend>Review weekdays</legend><div className={styles.weekdayGrid}>{dayLabels.map((day,index)=><label className={styles.check} key={day}><input type="checkbox" checked={weekdays.includes(index+1)} onChange={event=>setWeekdays(days=>event.target.checked?[...days,index+1].sort():days.filter(value=>value!==index+1))}/>{day}</label>)}</div></fieldset>}
