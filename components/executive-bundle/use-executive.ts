...
async function request(path:string,subject:string|undefined,signal:AbortSignal,input?:object) {
 ...
 if(!response.ok)throw new ReadError(typeof result.message==="string"?result.message:"Executive is unavailable. Please retry.",response.status);
 return result;
}
export function useExecutiveRead<T>(path:string,capability:string,body?:object,refreshOnFocus=false) {
 ...
 const key=enabled?user.id+":"+bundleExperience.workspaceId+":"+bundleExperience.revision+":"+path+":"+bodyText:null;
 ...
 useEffect(()=>{
  ...
  void request(path,user?.id,controller.signal,bodyText?JSON.parse(bodyText):undefined).then(data=>{if(live)setState({key,data,error:null});}).catch(error=>{
   if(!live)return;
   setState({key,data:null,error:controller.signal.aborted?"This request took too long. Try again; no saved work was changed.":error instanceof Error?error.message:"Executive is unavailable."});
   if(error instanceof ReadError&&[401,403].includes(error.status))refreshBundleExperience();
  }).finally(()=>clearTimeout(timer));
  return()=>{live=false;clearTimeout(timer);controller.abort();};
 },[key,path,user?.id,attempt,refreshBundleExperience,bodyText]);
 useEffect(()=>{
  if(!enabled||!refreshOnFocus)return;
  const refresh=()=>{setState(null);setAttempt(n=>n+1);};
...
export function useExecutiveAction() {
 ...
 const run=async<T,>(path:string,input:object|null,idempotent=false):Promise<T|null>=>{
  ...
  try {
   ...
   const result=await request(path,user?.id,control.signal,input===null?undefined:idempotent?{...input,requestId:attempt.current.requestId}:input);
...
export function useUnsavedExecutive(dirty:boolean) {
 useEffect(()=>{
  if(!dirty)return;
  const before=(event:BeforeUnloadEvent)=>{event.preventDefault();};
  const click=(event:MouseEvent)=>{
   const link=(event.target as Element)?.closest?.("a[href]") as HTMLAnchorElement|null;
   if(link&&link.target!=="_blank"&&!link.hasAttribute("download")&&!event.ctrlKey&&!event.metaKey&&!event.shiftKey&&link.href!==window.location.href&&!window.confirm("Leave without saving your Executive changes?")){event.preventDefault();event.stopPropagation();}
  };
