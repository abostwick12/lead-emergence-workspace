...
export function FirstValue({ onSave, preview }: { onSave: (command: Command) => Promise<void>; preview: boolean }) {
  ...
  return <section className={styles.firstValue}>
    <p className={styles.eyebrow}>SOTF Bundle · Start with your current question</p>
    <h1>You do not need the title yet.<br /><em>Start with the work you want to test.</em></h1>
    <p>We’ll keep your question, evidence, and next step together. A short answer is enough.</p>
    {preview ? <p className={styles.notice}>Fictional preview. Use invented information; nothing here is saved to an account.</p> : null}
    <ol className={styles.steps} aria-label="First session progress">{["Decision to work on", "What good looks like", "Directions to test"].map((label, index) => <li key={label} aria-current={step === index ? "step" : undefined}>{index + 1}<span>{label}</span></li>)}</ol>
    <form className={styles.form} onSubmit={(event) => void submit(event)}>
      {step === 0 ? <>
        ...
        <label>What decision would be most useful to work on?<textarea required maxLength={5000} value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="For example: which kind of work would let me own decisions?" /></label>
        ...
        <label>What work do you want to do?<textarea required maxLength={5000} value={work} onChange={(event) => setWork(event.target.value)} placeholder="Describe work you want to test—not a résumé summary." /></label>
        ...
        <p className={styles.help}>These become two criteria you can change as you learn. Keep private or sensitive material out of this operational pilot; its protected-context capability is separate.</p>
      </> : <>
        <p>Choose two possible directions to test. They are hypotheses, not commitments. Your connected ChatGPT conversation can suggest alternatives from the context you choose to share.</p>
        ...
        <p className={styles.notice}>Next: bring one real role. We’ll separate eligibility from fit, then choose the question worth answering.</p>
      </>}
      {error ? <p role="alert" className={styles.error}>{error}</p> : null}
      <div className={styles.formActions}>{step > 0 ? <button type="button" onClick={() => setStep(step - 1)} disabled={pending}>Back</button> : null}<button className={styles.primary} disabled={pending}>{pending ? "Verifying…" : step === 2 ? "Save these starting points" : "Continue"}</button></div>
    </form>
  </section>;
}
