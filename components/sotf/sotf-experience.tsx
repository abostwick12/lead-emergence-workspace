...
export function SotfExperience({ mode }: { mode: "preview" | "connected" }) {
  ...
  return <section className={styles.experience} aria-label="SOTF Bundle">
    <header className={styles.header}><a className={styles.brand} href="https://entry.leademergence.com">LEAD EMERGENCE<span>SOTF Bundle</span></a><div className={styles.headerTools}>{preview ? <span className={styles.previewLabel}>Fictional preview · resets on reload</span> : <span className={styles.previewLabel}>Your work, carried forward</span>}<button onClick={() => { setPanel("history"); setView("Today"); }}>History</button>{state.chapter ? <button onClick={exportState}>Export my work</button> : null}</div></header>
    {preview ? <div className={styles.previewBar}><p>This is a fictional working example. Nothing is sent, and no account data is read or saved.</p><button onClick={() => { setState(emptyPilotState()); setSelected(null); setPanel(null); setView("Today"); setMessage(""); }}>Start over</button></div> : null}
    ...
    {loading ? <p role="status" className={styles.empty}>Recovering the decision, the evidence, and what comes next…</p> : !state.chapter && !error ? <FirstValue onSave={save} preview={preview} /> : state.chapter ? <>
      ...
      <div aria-busy={busy} className={styles.body}>
        {view === "Today" ? <>
          <div className={styles.sectionHeading}><h2>{panel === "coaching" ? "Prepare for your SOTF session" : panel === "weekly" ? "What should change next week?" : panel === "stories" ? "Find an example to use" : panel === "history" ? "Recent decisions" : "For today"}</h2><div className={styles.actions}><button onClick={() => setPanel(panel === "coaching" ? null : "coaching")}>Prepare my SOTF session</button><button onClick={() => setPanel(panel === "weekly" ? null : "weekly")}>Review my week</button><button onClick={() => setPanel(panel === "stories" ? null : "stories")}>Find an example</button></div></div>
          {panel === "coaching" ? <div className={styles.twoColumns}><article className={styles.sheet}><p className={styles.eyebrow}>Since {displayDate(coach.since)}</p><h3>Bring this to your coach</h3><List items={coach.agenda} /><h4>Done since the last session</h4><List items={coach.completed.map((item) => item.title + " — " + item.result)} /><h4>Still stuck</h4><List items={coach.stuck.map((item) => item.title + " — " + item.result)} /><h4>What changed</h4><List items={coach.changed.map((item) => item.summary)} /><button onClick={() => edit("meeting")}>Record a coaching session</button></article><article className={styles.sheet}><p className={styles.eyebrow}>Draft to review before sharing</p><h3>{coach.shareable.summary}</h3><List items={[...coach.shareable.completed, ...coach.shareable.decisions, ...coach.shareable.nextExperiments]} /><p className={styles.help}>Private working notes stay out of this selection. Review the exact content and recipient before sharing.</p><button disabled={busy} onClick={() => edit("coach-share")}>Prepare the share draft</button></article></div>
          ...
          : panel === "stories" ? <><label className={styles.search}>What do you need to show?<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Stakeholder alignment, ambiguity, delivery…" /></label><button onClick={() => edit("story")}>Save a reviewed accomplishment</button><div className={styles.records}>{(query ? recallStories(state, query).map((item) => item.story) : state.stories).map((story) => <article className={styles.sheet} key={story.id}><p className={styles.eyebrow}>{story.skills.join(" · ")}</p><h3>{story.title}</h3><p>{story.approvedLanguage}</p><details><summary>Contribution and proof</summary><p>{story.situation}</p><p>{story.contribution}</p><p>Scope: {story.scope}</p><p>{story.actions}</p><p>{story.outcome}</p><List items={story.uncertainNumbers.map((item) => "Unverified: " + item)} /></details></article>)}</div></>
          : panel === "history" ? <div className={styles.timeline}>{[...state.changes].reverse().map((change) => <article key={change.id}><time>{displayDate(change.at)}</time><p>{change.summary}</p></article>)}<details><summary>Pick up in another conversation</summary><p>{resumeTransition(state).latestDecision?.opportunity.decision?.nextAction ?? "Start with the current question, criteria, and next experiment."}</p><p>ChatGPT&apos;s SOTF Bundle tools recover this operational history. They do not assume access to your other chats or protected context.</p></details></div>
          : <><div className={styles.brief}>{brief.length ? brief.map((item, index) => <article key={item.id}><span className={styles.ordinal}>0{index + 1}</span><div><p className={styles.eyebrow}>{item.whyNow}</p><h3>{item.title}</h3><p>{item.context}</p><p className={styles.help}>If you wait: {item.ifWait}</p><button onClick={() => { const [kind, ...rest] = item.id.split(":"); const id = rest.join(":"); if (kind === "commitment") edit("resolve-promise", id); else if (kind === "hypothesis") { setView("Direction"); } else if (kind === "deadline") { setView("Opportunities"); setSelected(id); } else { setView("People"); } }}>{item.action} <span aria-hidden="true">→</span></button></div></article>) : <p className={styles.empty}>Nothing needs an immediate transition action. Review a direction or bring an opportunity when it helps.</p>}</div><div className={styles.actions}><button className={styles.primary} onClick={() => edit("opportunity")}>Assess an opportunity</button><button onClick={() => edit("person")}>Prepare a conversation</button><button onClick={() => edit("promise")}>Keep a commitment</button></div></>}
        ...
        {view === "Direction" ? <><div className={styles.sectionHeading}><h2>Find the right work by testing it.</h2><button onClick={() => edit("hypothesis")}>Test another possibility</button></div><div className={styles.records}>{state.hypotheses.map((item) => { const learning = hypothesisLearning(state, item.id); return <article className={styles.sheet} key={item.id}><p className={styles.eyebrow}>{item.status} · {item.confidenceExplanation}</p><h3>{item.proposition}</h3><p>{item.whyPromising}</p><h4>Next experiment</h4><p>{item.nextExperiment}</p><p className={styles.help}>Review when: {item.reviewTrigger}</p><details><summary>Evidence, assumptions, and learning</summary><h4>Supporting</h4><List items={learning.supporting.map((entry) => entry.statement)} /><h4>Conflicting</h4><List items={learning.conflicting.map((entry) => entry.statement)} /><h4>Assumptions</h4><List items={item.assumptions} /><h4>Gaps to test</h4><List items={item.gaps} /><h4>Conversations and roles</h4><List items={[...learning.conversations.map((entry) => entry.title + " · " + entry.status), ...learning.jobsReviewed.map((entry) => entry.company + " · " + entry.status)]} /></details><button onClick={() => edit("hypothesis", item.id)}>Review this direction</button></article>; })}</div><div className={styles.sectionHeading}><h2>Your decision criteria</h2><button onClick={() => edit("criteria")}>Confirm a criterion</button></div>{state.criteria.map((item) => <article className={styles.row} key={item.id}><div><h3>{item.label}</h3><p>{item.desired}</p><span className={styles.help}>{item.nonNegotiable ? "Non-negotiable" : "Preference"} · importance {item.importance}/5</span></div><button onClick={() => edit("criteria", item.id)}>Revisit</button></article>)}</> : null}
        {view === "Opportunities" ? <>
          <div className={styles.sectionHeading}><h2>Is this worth more of your time?</h2><button className={styles.primary} onClick={() => edit("opportunity")}>Assess an opportunity</button></div>
          ...
          {activeOpportunity && assessment ? <>
            ...
            <List items={assessment.reasons} /><div className={styles.contextImpact}><p className={styles.eyebrow}>How your criteria affect the answer</p><List items={assessment.contextImpact} /></div>
