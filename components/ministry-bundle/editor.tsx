...
import {useMinistryRead,useMinistryAction,useUnsavedMinistry} from "./use-ministry";
...
function Editor({kind,document,reload}:{kind:Kind;document:MinistryDocument|null;reload:()=>void}) {
 const router=useRouter();const {bundleExperience}=useWorkspace();
 const [base,setBase]=useState(document),[value,setValue]=useState<NonNullable<MinistryDocument["data"]>>(document?.data??structuredClone(initial(kind)));
 const [confirm,setConfirm]=useState(false),[validation,setValidation]=useState<string|null>(null),[notice,setNotice]=useState<string|null>(null),[formKey,setFormKey]=useState(0),[restoreUnset,setRestoreUnset]=useState(false);
 const action=useMinistryAction();
 const canWrite=kind!=="research"||bundleExperience?.capabilityIds.includes("ministry.teaching")===true;
 const dirty=restoreUnset?base?.data!=null:JSON.stringify(value)!==JSON.stringify(base?.data??initial(kind));
 useUnsavedMinistry(dirty);
 const accept=(d:MinistryDocument)=>{setBase(d);setValue(d.data??structuredClone(initial(kind)));setRestoreUnset(false);setConfirm(false);setValidation(null);setFormKey(n=>n+1);setNotice("Saved revision "+d.revision+". Your earlier saved work is preserved.");};
 const save=async(clear=false)=>{
  ...
  const result=await action.run<{document:MinistryDocument}>("/api/ministry/"+kind,input,true);
  if(result){accept(result.document);if(!base&&kind!=="profile")router.replace("/workspace/ministry/"+kind+"/"+result.document.id);}
 };
 ...
 <fieldset disabled={!canWrite||action.busy} key={formKey}>
 {kind==="profile"?<ProfileFields value={value as TheologicalProfile} onChange={v=>{setValue(v);setRestoreUnset(false);setConfirm(false);}}/>:kind==="research"?<ResearchFields value={value as ResearchProject} onChange={setValue}/>:<ArchiveFields value={value as TeachingArchive} onChange={setValue}/>}
 ...
 <div className={styles.sticky}><span>{canWrite?"Save creates a recoverable revision.":"Research editing is not included in your access."}</span><button type="submit" disabled={!canWrite||action.busy||(kind==="profile"&&!confirm)}>{action.busy?"Saving…":kind==="profile"?"Confirm and save preferences":"Save "+(kind==="research"?"research":"teaching")}</button></div>
 {kind==="profile"&&base?.data&&<div className={styles.actions}><button type="button" disabled={!confirm||action.busy} onClick={()=>{if(window.confirm("Unset the current theological profile? Earlier saved versions remain recoverable."))void save(true);}}>Unset current preferences</button></div>}
 </fieldset></form>
