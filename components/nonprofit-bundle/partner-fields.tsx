...
export function PartnerFields({value,onChange,documentId}:{documentId?:string;value:PartnerRecord;onChange:(value:PartnerRecord)=>void}){
 ...
 <div id={documentId?taskTargetId("followup",documentId):undefined} tabIndex={-1} role="group" aria-label="Linked follow-up"><Field label="Next follow-up action" value={value.nextAction} multiline max={2000} onChange={v=>put("nextAction",v)}/>
 <div className={styles.grid}><Field label="Follow-up owner" value={value.owner} max={200} onChange={v=>put("owner",v)} hint="Leave blank until someone accepts responsibility."/><Field label="Follow-up date" type="date" value={value.followupDate??""} onChange={v=>put("followupDate",v||null)}/></div></div></section>
