...
- Vercel deployment/runtime-log API — **UNAVAILABLE FOR THIS PROJECT**. The connected Vercel integration lacks the `emergence-projects` project permission (404/403), and the signed-out Vercel dashboard cannot supply build/runtime logs. GitHub's deployment status is green and direct Preview HTTP/browser smoke passes, but runtime-error-log absence is not claimed.
