...
- Workspace PR [#4](https://github.com/abostwick12/lead-emergence-workspace/pull/4), runtime-bearing head `671b771` — **OPEN; VERCEL/PREVIEW COMMENTS SUCCESS**. The repair adds a content-free, exactly-once `first_capture_created` trigger and environment-aware MCP security fixtures. Git deployment `dpl_mZ7zLzCpP83piRZ38vddGyiDN2FB` is READY at `https://lead-emergence-workspace-puwqc97h0-emergence-projects.vercel.app`. A reviewed follow-up migration enables policy-free RLS on the three unexposed, grant-revoked private configuration tables; it passes locally and remains unapplied hosted pending explicit advisor approval. No GitHub Actions workflow exists in this repository; required checks were executed locally before push.
- Workspace protected branch Preview `https://lead-emergence-workspace-git-product-498b3c-emergence-projects.vercel.app` — **BUILD/AUTHENTICATED NATIVE LIFECYCLE/MOBILE/MCP METADATA PASS; INTERACTIVE OIDC REMAINS**. Deployment `dpl_C9JiXHrwoxMJ8ciASrqFUcEPqntU` is READY at `https://lead-emergence-workspace-b319lxsaf-emergence-projects.vercel.app`; the stable branch alias was moved to this accepted deployment. The build uses the isolated Personal sandbox and advertises the exact stable Preview MCP resource plus `https://nhkugzifuapplwpnfpbt.supabase.co/auth/v1`. Two synthetic `.invalid` users proved first entry, AI-first setup choice, ChatGPT and Claude instruction screens, native setup, save/resume, native-to-AI, AI-to-native fallback, first-value Home, returning-user bypass, plan display, no-connector usefulness, Quick Capture persistence, and desktop/mobile responsive behavior. Actual Entry OIDC and assistant consent remain unavailable until the Personal custom provider is created.
...
- Entry branch Preview `https://lead-emergence-entry-productization-emergence-projects.vercel.app` — **BUILD/PUBLIC ENTRY/JWKS/OAUTH CLIENT-ID CONFIG PASS; PERSONAL PROVIDER PENDING**. Git deployment `dpl_5VxCCtXm3VdsV1dYHsFdaT1UdfMH` is READY at `https://lead-emergence-entry-sso-preview-5s78c9op4-emergence-projects.vercel.app`; Next.js `16.3.1` compiled and type/static generation passed on Node `24.x`. Entry OAuth app `Lead Emergence Personal Workspace (preview)` was created with the exact Personal Supabase callback, its one-time secret was retained only in the authenticated browser session, and public client ID `80b81602-59c8-4d57-9d7f-6be0faf277f0` is present only in Entry Preview environment configuration. A synthetic `.invalid` Entry identity with ACTIVE Personal eligibility proves password login and the product chooser; it is retained only until real one-login acceptance and final cleanup.
...
- Vercel runtime logs — **PASS for both inspected runtime-bearing Previews**. Authenticated, bounded queries returned zero error-level runtime events and zero 5xx error events for Workspace deployment `dpl_C9JiXHrwoxMJ8ciASrqFUcEPqntU` after the hosted lifecycle and for Entry deployment `dpl_9ngWJuLsURVYaw9LNzFV5jD6ueXd`.
...
- Personal hosted migrations are **APPLIED/PASS**: `20260822121537_workspace_foundation`, `20260822121608_workspace_clock_preferences`, `20260822121616_workspace_productization`, test-only `20260822122741_enable_pgtap_acceptance`, `20260822123401_expose_workspace_data_api`, and `20260822124349_workspace_first_capture_event`. The cross-product Gate A hardening package was not applied because its only effect targets a Ministry-owned `public` policy that is absent and inapplicable in this isolated sandbox; the Supabase safety guard rejected that mutation.
- Hosted transactional pgTAP is **88/88 PASS**: 53 productization/plan/MCP assertions, 23 Workspace-only hostile cross-user/private-Storage assertions, 7 clock assertions, and 5 content-free first-capture analytics assertions. All fixtures rolled back; a verification query returned zero transactional synthetic users/Workspace rows/Storage objects. Separate browser fixtures remain intentionally isolated and are removed at final acceptance cleanup.
- Personal security advisors return no finding against `workspace`, `workspace_private`, or Workspace Storage. Existing findings are two informational rehearsal-table RLS notices, one pre-existing `public.current_ministry_id()` warning, and leaked-password protection disabled. The table inventory separately warns that three `workspace_private` tables do not enable RLS; the schema is not exposed and all anon/authenticated grants are revoked, so no ordinary Data API path exists. The optional defense-in-depth RLS SQL remains unapplied pending the explicit advisor-required decision.
- Personal performance advisors report 99 Workspace-relevant notices on the fresh sandbox: 51 per-row Auth helper evaluations, 40 uncovered foreign keys, and 8 indexes with no recorded use. Forward migration `20260822133500_workspace_advisor_performance.sql` rebuilds only the affected policies with identical semantics plus init-plan-safe Auth calls and adds only missing foreign-key covering indexes. A fresh local rebuild, 15/15 schema contracts, 93/93 pgTAP, and DB lint pass. The eight unused-index notices are not acted on because a new sandbox has no representative workload history. Hosted application was rejected by the repository gate and is not retried pending explicit written approval.
- Entry Preview `ENTRY_PERSONAL_OAUTH_REDIRECT_ORIGIN` targets `https://nhkugzifuapplwpnfpbt.supabase.co`. Workspace Preview has `ENTRY_OIDC_PROVIDER=custom:lead-emergence-entry-workspace-preview`. The Entry OAuth app and Entry Preview client-ID configuration are complete; the Personal custom provider still requires a user-authenticated dashboard session so its one-time secret can be entered directly without disclosure.
...
- actual Entry-to-Workspace OIDC and ChatGPT/Claude OAuth — Personal custom-provider/Auth configuration, custom access-token hook, and interactive consent are required;
