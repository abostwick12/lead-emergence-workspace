...
- Entry PR [#2](https://github.com/abostwick12/lead-emergence-entry/pull/2), head `a8bc959f1d4103a70f2e46a4c6f9d0babf34a748` — **OPEN, MERGEABLE/CLEAN; ENTRY QUALITY SUCCESS**. Final workflow run `32562955084`, job/check `97007275594`, passed application verification, isolated Supabase startup, schema lint, 44 pgTAP assertions, browser password-recovery acceptance, evidence upload, and disposable database cleanup in 3m57s. The runtime source pins Node from `>=24` to `24.x`; local typecheck, lint, 15/15 unit tests, production build, lock dry-run, full audit, and diff check also pass.
- Entry branch Preview `https://lead-emergence-entry-productization-emergence-projects.vercel.app` — **BUILD/PUBLIC ENTRY/JWKS PASS; OAUTH CLIENT BLOCKED**. Corrected deployment `dpl_9ngWJuLsURVYaw9LNzFV5jD6ueXd` is READY at unique URL `https://lead-emergence-entry-sso-preview-a3or8kfk2-emergence-projects.vercel.app`; Next.js `16.3.1` compiled and type/static generation passed on Node `24.x` without the prior broad-major runtime warning. The login renders, JWKS exposes one signing key, the stable branch alias uses the exact Preview `APP_ORIGIN`, and the Personal OAuth redirect setting now targets the dedicated Personal Supabase Auth origin rather than the Workspace application origin. The Preview-specific Vercel project remains separate from the newly provisioned Production authority.
- Dedicated Entry Vercel authority — **PROJECT/DNS/TLS/BASE CONFIG PASS; DEPLOYMENT BLOCKED**. Project `lead-emergence-entry` (`prj_Sjv0ZfqFzf7dOOime4bEWZukmaCD`) is connected to the Entry repository, uses `main` as its eventual production branch, and is configured as Next.js on Node `24.x`. Cloudflare Domain Connect installed a DNS-only project-specific CNAME and verification TXT record; Vercel reports `https://entry.leademergence.com` configured correctly and verified. Cloudflare's public resolver returns the project-specific CNAME, and a hostname-valid HTTPS probe reaches Vercel with HSTS. Its intentional `404 DEPLOYMENT_NOT_FOUND` proves no application or traffic has been assigned yet. Exact public Production origins plus a new environment-specific RSA handoff keypair/redemption secret are stored in Vercel. Supabase credentials and OAuth client IDs remain absent.
...
- Personal sandbox `nhkugzifuapplwpnfpbt` is **INACTIVE**. Its earlier restore attempt failed safely with the `EMERGEnce` organization's two-active-Free-project limit. After approval of the recommended isolated path, pre-mutation inventory found that the only active non-production project in that organization is named `lead-emergence-meridian-sandbox` (`lpqgjnuvfvuuashcmlxq`), not a Consulting development sandbox; Consulting's deployed environment points to the shared production project. No project was paused or restored under the stale label, and no paid branch or upgrade was created.
...
- Entry Preview `ENTRY_PERSONAL_OAUTH_REDIRECT_ORIGIN` targets the dedicated Personal sandbox Supabase Auth origin, `https://nhkugzifuapplwpnfpbt.supabase.co`; the application branch URL is not an OAuth callback origin. OAuth client/provider creation is authorized, but Workspace `ENTRY_OIDC_PROVIDER` and Entry `ENTRY_PERSONAL_OAUTH_CLIENT_ID` remain unset until the exact Personal sandbox restore target is confirmed. No placeholder or reused credential was added.
- Entry production identity authority — **BACKEND NOT PROVISIONED**. The isolated Entry development backend `vnjdubrnmxvmsccxmhst` is active in the separate Free organization `Lead emergence sandbox`; creating one additional project there currently reports `$0/month`. The Vercel authority is ready, but a user-selected production Supabase organization/project, reviewed Entry migrations, exact Auth/OAuth configuration, and synthetic acceptance remain required before deployment. `https://www.leademergence.com/login` remains the Ministry application.
