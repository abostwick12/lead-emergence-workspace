...
Status: **local and isolated hosted native candidate validated; the two reviewed advisor migrations are applied and postflight-validated on the Personal sandbox only; Workspace and Entry PRs remain open; real Entry OIDC/MCP consent, distinct production Entry backend capacity, and final PR checks remain; production cutover is not approved**.
