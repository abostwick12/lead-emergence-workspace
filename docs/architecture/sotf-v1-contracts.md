...
| `sotf_get_daily_brief_state` (new) | `{workflow_id:"transition.daily_brief", workflow_version:"1.0.0", brief_date, time_zone}` | `{projection,authority}` where `projection` is version `1` below and `authority` is the database-owned consistency envelope; no arbitrary selectors/query/SQL |
...
The successful `data` object is a closed `{projection,authority}` wrapper. `projection` contains `projection_version:"1"`, `workspace_id` (server UUID), `workflow_id`, `workflow_version`, `state_revision` (ordinary SOTF integer revision 0–2000), `as_of` (server UTC timestamp), `brief_date`, `time_zone`, database-derived `window_start` and `window_end`, `chapter`, `criteria`, `opportunities`, `commitments`, `meetings`, `hypotheses`, `suggestions`, `recent_outcomes`, `truncated_sections`, and `omitted_counts`. `authority` contains only `authority_version:"1"`, `authority_token`, and `authority_local_day`. Unknown fields are rejected. All reads belong to the same authorized workspace; the operational projection uses one consistent revision. Empty arrays are valid and distinct from unavailable state.
...
Every text comparison used to select or order a bounded projection member uses one canonical v1 ordering rule: compare the unsigned UTF-8 bytes of the decoded, well-formed string lexicographically. Do not normalize identifiers. Primary numeric/date/time/rank fields retain the ordering stated above; the immutable unique ID, compared by the same byte rule, is the final tie-breaker. PostgreSQL must compare an explicit byte key rather than inherit a database collation, and JavaScript must compare encoded bytes rather than call `localeCompare`. This rule determines exact bounded membership and therefore governed-reference eligibility, not merely presentation. Given identical durable state, both authorities must derive the same ordered projected IDs, omitted tail, truncation metadata, suggestions, and eligible-reference set regardless of locale, collation, ICU version, or insertion order.

Authority-sensitive SOTF v1 identifiers use exact decoded-string matching. A host must copy workflow IDs, workflow versions, IANA time-zone identifiers, and projected `selected_le_refs` values exactly as returned or documented. The application and authenticated RPC reject leading or trailing whitespace, case changes, Unicode normalization, and visually similar substitutions; neither boundary silently transforms the submitted identity. Thus `"a-b"` may match a projected identifier while `" a-b "` is invalid. This boundary is intentionally stricter than natural-language input: selected references are machine identities from the bounded projection, so forgiving parsing would create aliases and could collapse distinct canonical IDs. JSON escape spelling may differ only when it decodes to the identical string.

Time zones have an additional closed membership contract because JavaScript and PostgreSQL expose different tzdb compatibility aliases. SOTF v1 freezes the 418 identifiers shared by the pinned runtimes from IANA 2025b `zone.tab` primary identifiers plus `UTC`. The IANA primary `America/Coyhaique` is excluded because the pinned PostgreSQL catalog cannot execute it. Links and implementation names—including `posix/America/Chicago`, `US/Central`, `CST6CDT`, `Etc/UTC`, `Asia/Calcutta`, `Europe/Kiev`, and `America/Godthab`—are invalid even if one runtime accepts them. The application and database use the same version-controlled exact set; neither canonicalizes an alias or rewrites the submitted identifier. A future identifier-set change requires an explicit contract revision and parity migration, not ambient runtime discovery.

Identifier membership and civil-time interpretation are separate contracts. The canonical identifier list answers only whether a submitted name is allowed. Authority-sensitive local-day and daily-window UTC boundaries are computed by PostgreSQL and returned in the authority result; the live MCP projection consumes those returned instants and never recreates them with Node/ICU. Host time-zone APIs may format display text only. The authority token is a non-secret optimistic fingerprint over the database local day, time-zone rules, revision, boundaries, eligible references, and truncation state. A first write supplies the exact token separately from the durable 19-field outcome; PostgreSQL recomputes it under the write lock. A changed revision, local day, rule set, eligible-reference set, or truncation state returns `state_changed` and requires a fresh state read and renewed review. The client never submits boundaries for PostgreSQL to trust.

Timestamp precision follows the same single-authority rule. PostgreSQL validates persisted temporal fields and positive meeting intervals at native microsecond precision, retains their exact strings, and evaluates bounded overlap. Canonical application replay validates structure and preserves those strings opaquely; it never converts them through JavaScript `Date` to decide ordering, duration, equality, membership, or validity. The authorized bounded-state RPC returns the complete database-built projection plus a projection fingerprint and authority token. Its eligible-reference membership is authoritative. MCP validates the projection's closed structure and exact request/workspace/revision identity, then transports it without rebuilding candidate membership, ordering, truncation, omitted counts, or byte-bound selection. A display layer may format both ends of a sub-millisecond interval to the same millisecond, but that visual equality has no governed meaning. Direct authenticated outcome writes independently recompute current PostgreSQL membership and authority before persistence.
...
| Expected input | Strict MCP outcome schema with a database authority token; the separately persisted outcome remains the closed 19-field, maximum 8 KiB UTF-8 payload; semantic checks below; reject unknown properties |
...
Semantic validation requires exact membership in the canonical SOTF v1 time-zone set, a brief date within the database-local today/yesterday bound, the released version, `expected_state_revision` matching the ordinary state snapshot, and `expected_authority_token` matching a fresh database recomputation for a first insert. The host retains the exact returned token only for the reviewed invocation. It is not an entitlement lease and is never persisted in the outcome payload. Recompute projection eligibility against the submitted date and current database authority. `selected_le_refs` must be unique, exist in the returned-eligible projection for that workspace/revision, and number no more than `priority_count` (0–3). The workflow may prioritize host-only information, so refs may be fewer than priorities. No refs can point to protected records.
...
Canonical ordering and canonical text measurement are separate contracts. UTF-16 code units remain the truncation unit; UTF-8 bytes are used only as a deterministic ordering key. Neither contract normalizes text, and composed/decomposed identifiers remain distinct identities.
