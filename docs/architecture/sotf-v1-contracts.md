...
Timestamp precision follows the same single-authority rule. PostgreSQL retains event timestamp strings and evaluates meeting overlap as `timestamptz` at its native microsecond precision. Application replay may expose a presentation-normalized timestamp at JavaScript's millisecond precision, but that representation never determines governed membership. When database authority is present, its exact eligible-reference membership is authoritative for every reference-bearing bounded section; the application uses those IDs to assemble the response and verifies the resulting set before returning it. Thus a meeting ending at `.000500Z` may display as `.000Z` after application replay without becoming ineligible. Direct authenticated outcome writes independently recompute the current PostgreSQL membership and authority token before persistence.
