...
export default function OAuthConsentPage() {
  ...
  return <main className="auth-page"><section className="auth-card consent-card">
    ...
    <div className="consent-list"><p><Check size={16} />Read your onboarding state, confirmed configuration, tasks, Quick Captures, personal memory, career opportunities, integration connection status, and governed professional context.</p><p><Check size={16} />Save your exact user-reported setup and propose interpretations for review. Professional observations remain candidates until your authorized assistant records an approve, correct, reject, or conflict-supersession decision; approval and supersession accept the candidate exactly, while correction records an actual change.</p><p><Check size={16} />Create or update internal Workspace records only when you explicitly ask. The controlled tools require the assistant to attest to that request; independently verified confirmation receipts for Professional Context are not yet released.</p><p><Check size={16} />Private and sensitive professional context requires explicit protected-context access. Do not submit classified, CUI, or operationally sensitive material.</p><p><Check size={16} />This approval does not connect external services, reveal connector credentials, send messages, or create calendar events. Those actions require separate provider consent and confirmation.</p></div>
