...
export default function IntegrationsPage() {
  ...
  return <><p className="eyebrow workflow-kicker">Connection states</p><h1 className="page-title">Integrations</h1><p className="page-lede">Connections are shown only when this Workspace is ready to use them.</p>{error ? <p className="error">{error}</p> : null}<section className="grid two">{catalog.map(([provider, label, description]) => { const connection = connections.find((item) => item.provider === provider); const status = connection?.status ?? "reconnect_required"; return <article className="card" key={provider}><div className="row"><h2>{label}</h2><span className={`pill ${status}`}>{status.replaceAll("_", " ")}</span></div><p className="muted">{description}</p><p className="notice">{connection ? "Reconnect this service when it is ready for use." : "This service is not connected yet."}</p></article>; })}</section></>;
}
