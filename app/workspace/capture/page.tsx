...
export default function CapturePage() {
  ...
  if (!capabilityEnabled(capabilities, "quick_capture", plan?.status)) return <CapabilityLockedState title="Quick Capture" benefit="Quick Capture keeps a signal until you decide what it means and what to do next." suspended={plan?.status === "suspended"} />;
  return <><p className="eyebrow workflow-kicker">Signals</p><h1 className="page-title">Quick capture</h1><p className="page-lede">Catch an idea now, then deliberately turn it into a task or discard it.</p>
    ...
    <section className="card" style={{ marginTop: 18 }}><h2>Inbox</h2>{error ? <p className="error">{error}</p> : null}{entries.filter((entry) => entry.status === "unprocessed").length ? entries.filter((entry) => entry.status === "unprocessed").map((entry) => <article className="item" key={entry.id}><p>{entry.raw_text}</p><div className="row"><span className="muted">Captured {new Date(entry.created_at).toLocaleDateString()}</span><div className="row"><button className="button" onClick={() => void resolve(entry)}>Make task</button><button className="button secondary" onClick={() => void dismissCapture(entry.id).then(reload).catch((caught) => setError(caught instanceof Error ? caught.message : "Could not discard capture."))}>Discard</button></div></div></article>) : <p className="empty">Capture a signal before deciding what it means. Write the thought above; you can turn it into a task or discard it when you review the inbox.</p>}</section>
  </>;
}
