...
select is((select count(*)::integer from workspace.bundle_capabilities where bundle_key = 'sotf_transition'), 5, 'SOTF receives only the ordinary pilot capability mappings');
select is((select count(*)::integer from workspace.bundle_capabilities where bundle_key = 'sotf_transition' and capability_key = 'professional_context'), 0, 'the bundle foundation does not add Professional Context');
