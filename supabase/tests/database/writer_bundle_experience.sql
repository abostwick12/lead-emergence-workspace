...
select is(jsonb_array_length(workspace.get_bundle_experience()->'capabilities'),2,'Writer receives the two activated portable capabilities');
...
set local role authenticated;
select is(jsonb_array_length(workspace.get_bundle_experience()->'capabilities'),1,'disabled mapping removes the individual capability');
...
select is(jsonb_array_length(workspace.get_bundle_experience()->'capabilities'),2,'MCP resolves the same two capabilities');
