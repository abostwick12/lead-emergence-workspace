...
select is((select enabled from workspace.bundle_capabilities where bundle_key = 'sotf_transition' and capability_key = 'professional_context'), true, 'SOTF bundle includes the graph capability');
...
select is(has_function_privilege('anon', 'workspace.mcp_get_context_provenance(uuid)', 'execute'), false, 'anonymous callers cannot inspect context provenance');
...
select set_config('request.identity_candidate_id', workspace.mcp_propose_context_candidate(
...
select is(workspace.mcp_propose_context_candidate(
  request_id => 'a1910000-0000-4000-8000-000000000001',
  target_family => 'professional_identity', proposed_label => 'Operational leader',
  proposed_summary => 'Translates complex operations into clear team execution.',
  proposed_tier => 'chapter', target_chapter_key => 'sotf_transition',
...
select set_config('request.identity_entity_id', workspace.mcp_review_context_candidate(
  current_setting('request.identity_candidate_id')::uuid, 'approve',
  'a1920000-0000-4000-8000-000000000001', 'chapter', null, null,
  'sotf_transition', 'User confirmed during intake.'
) #>> '{context,id}', true);
...
select set_config('request.goal_candidate_id', workspace.mcp_propose_context_candidate(
...
select lives_ok(format(
  'select workspace.mcp_review_context_candidate(%L::uuid, %L, %L::uuid, %L, null, null, %L, %L)',
  current_setting('request.goal_candidate_id'), 'approve', 'a1920000-0000-4000-8000-000000000002',
  'chapter', 'sotf_transition', 'Goal confirmed.'
), 'initial career goal can be confirmed through bounded review');
select set_config('request.direction_candidate_id', workspace.mcp_propose_context_candidate(
...
select lives_ok(format(
  ...
  'chapter', 'sotf_transition', 'Direction confirmed.'
), 'career direction can be confirmed');
select set_config('request.criterion_candidate_id', workspace.mcp_propose_context_candidate(
...
select lives_ok(format(
  'select workspace.mcp_review_context_candidate(%L::uuid, %L, %L::uuid, %L, null, null, null, %L)',
  current_setting('request.criterion_candidate_id'), 'approve', 'a1920000-0000-4000-8000-000000000004',
  'core', 'Criterion confirmed.'
...
select set_config('request.learning_candidate_id', workspace.mcp_propose_context_candidate(
...
select set_config('request.learning_entity_id', workspace.mcp_review_context_candidate(
  current_setting('request.learning_candidate_id')::uuid, 'correct',
  'a1920000-0000-4000-8000-000000000005', 'chapter', null,
  'Quantify team size, operating tempo, and outcomes in interview examples.',
  'sotf_transition', 'User amended the coaching lesson.'
) #>> '{context,id}', true);
...
select set_config('request.preference_candidate_id', workspace.mcp_propose_context_candidate(
...
select set_config('request.preference_entity_id', workspace.mcp_review_context_candidate(
  current_setting('request.preference_candidate_id')::uuid, 'approve',
  'a1920000-0000-4000-8000-000000000006', 'core', null, null, null, 'Initial preference confirmed.'
) #>> '{context,id}', true);
select set_config('request.conflict_candidate_id', workspace.mcp_propose_context_candidate(
...
select throws_ok(format(
  'select workspace.mcp_review_context_candidate(%L::uuid, %L, %L::uuid, %L)',
  current_setting('request.conflict_candidate_id'), 'approve', 'a1920000-0000-4000-8000-000000000007', 'core'
), '22023', 'Conflicting context must be explicitly superseded or rejected.', 'generic approval cannot bypass conflict resolution');
select set_config('request.replacement_preference_id', workspace.mcp_review_context_candidate(
  current_setting('request.conflict_candidate_id')::uuid, 'supersede',
  'a1920000-0000-4000-8000-000000000008', 'core', null, null, null,
...
select ok(pg_catalog.jsonb_array_length(workspace.mcp_get_context_provenance(current_setting('request.preference_entity_id')::uuid) -> 'conflicts') = 1, 'provenance exposes the conflict and its review history');
...
select is(workspace.mcp_link_professional_context(
...
select throws_ok(format(
  'select workspace.mcp_link_professional_context(%L::uuid, %L, %L::uuid, null, %L, %L::uuid)',
...
select set_config('request.private_candidate_id', workspace.mcp_propose_context_candidate(
...
select lives_ok(format(
  'select workspace.mcp_review_context_candidate(%L::uuid, %L, %L::uuid, %L, null, null, %L)',
  current_setting('request.private_candidate_id'), 'approve', 'a1920000-0000-4000-8000-000000000009',
  'chapter', 'sotf_transition'
), 'private context can be explicitly reviewed and confirmed');
...
select throws_ok(
  ...
  '42501', 'Private context requires explicit access confirmation.',
...
select set_config('request.sensitive_candidate_id', workspace.mcp_propose_context_candidate(
...
select lives_ok(format(
  'select workspace.mcp_review_context_candidate(%L::uuid, %L, %L::uuid, %L)',
  current_setting('request.sensitive_candidate_id'), 'approve', 'a1920000-0000-4000-8000-000000000010', 'core'
