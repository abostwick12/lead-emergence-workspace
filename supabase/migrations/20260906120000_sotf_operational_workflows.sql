...
begin
  if auth.uid() is null then raise exception 'Sign in to SOTF Bundle.' using errcode = '42501'; end if;
  if auth.jwt() ->> 'client_id' is not null then
    target_workspace := workspace_private.require_mcp_workspace();
  else
    if not workspace_private.is_direct_session() then raise exception 'A current Workspace session is required.' using errcode = '42501'; end if;
  ...
  if target_workspace is null
    ...
    ) then raise exception 'Active SOTF Bundle access is required.' using errcode = '42501'; end if;
