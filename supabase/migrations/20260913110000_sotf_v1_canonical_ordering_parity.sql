-- Canonical SOTF v1 authority ordering: unsigned UTF-8 byte lexicographic order.
-- bytea comparison bypasses database collation and matches the application helper.
create function workspace_private.sotf_v1_order_key(value text)
returns bytea language sql immutable strict parallel safe set search_path = '' as $$
  select pg_catalog.convert_to(value,'UTF8');
$$;

revoke all on function workspace_private.sotf_v1_order_key(text) from public,anon,authenticated;

comment on function workspace_private.sotf_v1_order_key(text) is
  'Returns the locale-independent UTF-8 byte key used for SOTF v1 authority-sensitive bounded ordering.';

-- The application reads only the most recent three receipts before rebuilding
-- its bounded projection. The membership of that pre-limited set is therefore
-- part of the same authority contract as the projection function below.
create or replace function workspace.sotf_v1_list_daily_brief_outcomes(p_workflow_version text)
...
begin
  ...
  return coalesce((
    select jsonb_agg(row.receipt order by row.recorded_at desc, workspace_private.sotf_v1_order_key(row.id::text) desc)
    from (
      ...
      order by item.recorded_at desc, workspace_private.sotf_v1_order_key(item.id::text) desc
      limit 3
    ) as row
  ), '[]'::jsonb);
end; $$;
...
begin
  ...
  ) order by (row ->> 'importance')::integer desc,workspace_private.sotf_v1_order_key(row ->> 'id')),'[]'::jsonb)
    into criteria_rows from (
      select value as row
      from jsonb_array_elements(criteria)
      order by (value ->> 'importance')::integer desc,workspace_private.sotf_v1_order_key(value ->> 'id')
      limit 20
    ) as source;
  ...
  select coalesce(jsonb_agg(row order by workspace_private.sotf_v1_order_key(row ->> 'deadline'),workspace_private.sotf_v1_order_key(row ->> 'id')),'[]'::jsonb) into opportunity_rows
  from (select jsonb_build_object(
    ...
    where entry.value ->> 'status' not in ('decline','pause')
      and nullif(entry.value ->> 'deadline','') is not null
      and (entry.value ->> 'deadline')::date < end_date
    order by workspace_private.sotf_v1_order_key(entry.value ->> 'deadline'),workspace_private.sotf_v1_order_key(entry.value ->> 'id') limit 10) as source;
  ...
  select coalesce(jsonb_agg(row order by workspace_private.sotf_v1_order_key(row ->> 'due'),workspace_private.sotf_v1_order_key(row ->> 'id')),'[]'::jsonb) into commitment_rows
  from (select jsonb_build_object(
    ...
    where entry.value ->> 'status' in ('open','blocked')
      and nullif(entry.value ->> 'due','') is not null
      and (entry.value ->> 'due')::date < end_date
    order by workspace_private.sotf_v1_order_key(entry.value ->> 'due'),workspace_private.sotf_v1_order_key(entry.value ->> 'id') limit 10) as source;
  ...
  select coalesce(jsonb_agg(row order by workspace_private.sotf_v1_order_key(row ->> 'starts_at'),workspace_private.sotf_v1_order_key(row ->> 'id')),'[]'::jsonb) into meeting_rows
  from (select jsonb_build_object(
    ...
    where entry.value ->> 'status' in ('planned','accepted')
      and (entry.value ->> 'startsAt')::timestamptz < window_end
      and (entry.value ->> 'endsAt')::timestamptz > window_start
    order by workspace_private.sotf_v1_order_key(entry.value ->> 'startsAt'),workspace_private.sotf_v1_order_key(entry.value ->> 'id') limit 10) as source;
  ...
  select coalesce(jsonb_agg(row order by workspace_private.sotf_v1_order_key(row ->> 'id')),'[]'::jsonb) into hypothesis_rows
  from (select jsonb_build_object(
    ...
    where entry.value ->> 'status' in ('continue','refine')
    order by workspace_private.sotf_v1_order_key(entry.value ->> 'id') limit 3) as source;

  if workspace_private.sotf_v1_text_units(chapter ->> 'question') > 500 then truncated := array_append(truncated,'chapter'); end if;

  select coalesce(jsonb_agg(receipt order by recorded_at desc,workspace_private.sotf_v1_order_key(id::text) desc),'[]'::jsonb) into recent_outcomes
  from (
    ...
    order by outcome_row.recorded_at desc,workspace_private.sotf_v1_order_key(outcome_row.id::text) desc limit 3
  ) as rows;

  select coalesce(jsonb_agg(value order by rank,workspace_private.sotf_v1_order_key(sort_key)),'[]'::jsonb) into suggestions
  from (
    select candidate.value,candidate.rank,candidate.sort_key
    from (
    ...
    order by rank,workspace_private.sotf_v1_order_key(sort_key) limit 3
  ) as ranked;
  ...
  foreach section in array array['recent_outcomes','hypotheses','meetings','commitments','opportunities','criteria'] loop
    ...
    loop
      ...
      projection := jsonb_set(projection,array['truncated_sections'],
        to_jsonb((select coalesce(array_agg(name order by workspace_private.sotf_v1_order_key(name)),'{}'::text[]) from (select distinct name from unnest(truncated) name) names)));
    end loop;
  end loop;

  select coalesce(jsonb_agg(candidate.value order by candidate.ordinality),'[]'::jsonb) into suggestions
  from jsonb_array_elements(projection -> 'suggestions') with ordinality as candidate(value,ordinality)
  where exists (
    select 1 from (
  ...
  select coalesce(jsonb_agg(jsonb_build_object('entity_type',kind,'entity_id',id) order by workspace_private.sotf_v1_order_key(kind),workspace_private.sotf_v1_order_key(id)),'[]'::jsonb)
