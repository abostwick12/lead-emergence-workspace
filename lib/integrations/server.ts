...
export async function saveIntegrationCredential(input: {
  ...
  const ciphertext = input.status === "error" ? null : encryptIntegrationValue(
    JSON.stringify(input.credential),
    secret,
    `${input.workspaceId}:${provider.credentialFamily}`
  );
  const expiresAt = typeof input.credential.expires_in === "number"
    ? new Date(Date.now() + input.credential.expires_in * 1000).toISOString()
    : null;
