...
export function registerSotfTools(server: McpServer, client: SupabaseClient<any, any, any, any, any>) {
  ...
  server.registerTool("sotf_prepare_next_move", {
    ...
    inputSchema: { workflow: z.enum(["opportunity", "meeting", "coaching", "direction", "weekly", "daily", "stories", "interview", "offers", "professional_work"]), recordId: z.string().max(100).optional(), query: z.string().max(1000).optional(), since: z.string().datetime({ offset: true }).optional() }, annotations: readOnly, ...contract
  }, ({ workflow, recordId, query, since }) => result(async () => {
