...
export function investorQuery(request:Request,keys:string[]) {
  const params=new URL(request.url).searchParams;
  if([...params.keys()].some(k=>!keys.includes(k)||params.getAll(k).length!==1)) throw new BundleApiError("Use only the requested Investor fields.",400);
  return params;
}
export async function investorHttp(request:Request,operation:(client:Client,input?:unknown)=>Promise<unknown>,mutation=false) {
  try {
    ...
    if(mutation) {
      ...
      try {for(;;){const {done,value}=await reader.read();if(done)break;size+=value.byteLength;if(size>650000){await reader.cancel();throw new BundleApiError("This record is too large. Shorten the public-research notes or split the record.",413);}chunks.push(value);}}
    ...
    return Response.json({message:error instanceof BundleApiError?error.message:status===400?"Check the Investor record, sources, and revision.":"Investor is temporarily unavailable. You can safely retry."},{status,headers});
  }
}
